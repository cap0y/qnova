...
export default function SeminarManagement({ user, selectedPreset: propSelectedPreset, questionCount: propQuestionCount }: SeminarManagementProps) {
  ...
  // Preset State - props로 받으면 props 우선, 없으면 내부 상태
  const [_selectedPreset, _setSelectedPreset] = useState<string | null>(null);
  const [_questionCount, _setQuestionCount] = useState<number>(10);
  const selectedPreset = propSelectedPreset !== undefined ? propSelectedPreset : _selectedPreset;
  const questionCount = propQuestionCount !== undefined ? propQuestionCount : _questionCount;
  const setSelectedPreset = _setSelectedPreset;
  const setQuestionCount = _setQuestionCount;
